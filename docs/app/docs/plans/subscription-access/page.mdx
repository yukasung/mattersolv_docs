...
MatterSolv ใช้แนวคิดจาก Odoo เรื่อง recurring plan, online checkout,
Subscription lifecycle และ customer account มาปรับเป็น flow ของผลิตภัณฑ์
MatterSolv เอง ลำดับหน้าจอของ Odoo แต่ละระบบอาจต่างกันตามการตั้งค่า
...
flowchart TD
  ...
  B -->|แผนเสียเงินแบบ Self-service| P1["เลือกแผนและรอบบิล"]
  P1 --> P2["กรอกข้อมูลผู้ซื้อ สำนักงาน<br/>ที่อยู่เรียกเก็บเงิน และข้อมูลภาษี"]
  P2 --> P3["ตรวจยอดและชำระผ่าน Hosted Checkout"]
  P3 --> P4{"ได้รับ webhook ที่ตรวจสอบแล้วหรือไม่"}
  P4 -->|ยังไม่ได้หรือไม่สำเร็จ| P5["แสดง Pending หรือ Failed<br/>และให้ลองชำระใหม่"]
  P5 --> P3
  P4 -->|ได้รับและสำเร็จ| P6["สร้าง Subscription, Workspace<br/>และคำเชิญ Owner"]
  P6 --> P7["ส่งอีเมลให้ Owner ตั้งรหัสผ่าน"]
  P7 --> P8["เปิดใช้งาน User"]
  P8 --> Onboarding

  B -->|ENTERPRISE หรือขอใบเสนอราคา| Q1["ส่งคำขอใบเสนอราคา"]
  Q1 --> Q2["ฝ่ายขายจัดทำและส่งใบเสนอราคา"]
  Q2 --> Q3["ลูกค้ายอมรับและชำระเงิน<br/>หรือยืนยันเงื่อนไขเครดิต"]
  Q3 --> P6

  Provision --> Onboarding["Onboarding"]
  Onboarding --> O1["ยืนยันข้อมูลสำนักงานและตั้งค่าเริ่มต้น"]
...
สร้าง Workspace เพราะยังไม่มีการชำระเงินเป็นสัญญาณยืนยัน หลังเปิดลิงก์ ผู้สมัคร
ตั้งรหัสผ่าน แล้วระบบสร้าง User, Workspace, Owner Membership และ Subscription
พร้อมกัน ผู้สมัครจึงเข้าสู่ Onboarding ได้ แผนฟรีมีสถานะ `active` ส่วน Trial
มีสถานะ `trialing` และวันหมดอายุตามเงื่อนไขที่กำหนด

ระบบต้องใช้ลิงก์แบบครั้งเดียวและมีอายุจำกัด หากลิงก์หมดอายุ ผู้สมัครขอส่งใหม่ได้
โดยยังไม่ได้สิทธิ์เข้าถึง Workspace จนกว่ายืนยันสำเร็จ

### Self-service Paid Plan
...
จากนั้นจึงส่งอีเมลให้ Owner ตั้งรหัสผ่าน ซึ่งเป็นการยืนยันว่า Owner เข้าถึงอีเมล
นั้นได้

### Enterprise or Quotation-assisted Plan

ลูกค้าส่งข้อมูลสำนักงาน ผู้ติดต่อ จำนวนสมาชิกโดยประมาณ และความต้องการให้ฝ่ายขาย
จัดทำใบเสนอราคา เมื่อลูกค้ายอมรับและชำระเงินหรือได้รับอนุมัติเงื่อนไขเครดิตแล้ว
ระบบใช้ขั้นตอนสร้าง Workspace และ Owner ชุดเดียวกับแผน Self-service
...
| Membership | สร้าง Workspace | เชื่อม User คนแรกเป็น Owner และสมาชิกที่เชิญภายหลัง |
...
- หากลิงก์เปิดใช้ Owner หมดอายุ ต้องออกลิงก์ใหม่ได้หลังตรวจความสัมพันธ์กับ
...
- [Odoo Subscriptions](https://www.odoo.com/documentation/18.0/applications/sales/subscriptions.html)
- [Odoo Customer Accounts](https://www.odoo.com/documentation/18.0/applications/websites/ecommerce/customer_accounts.html)
- [Odoo Automatic Payments](https://www.odoo.com/documentation/18.0/applications/sales/subscriptions/automatic_payments.html)

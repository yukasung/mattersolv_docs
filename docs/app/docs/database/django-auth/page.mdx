...
และ tenant_role_assignments ไม่อยู่ในขอบเขตของหน้านี้ แต่มีคำอธิบายรายคอลัมน์อยู่ที่
[Tenant & Authorization Tables](/docs/database/tenant-authorization) ส่วนเส้นทาง
การตรวจสิทธิ์ที่เชื่อมสองฝั่งเข้าด้วยกันอธิบายไว้ที่หัวข้อ Permission Resolution Path
...
คำว่า ระดับระบบ ในหน้านี้หมายถึงสิทธิ์ที่ไม่ผูกกับสำนักงานใดสำนักงานหนึ่ง และมีผล
กับผู้ใช้ทุกที่ในระบบ ต่างจากสิทธิ์ระดับสำนักงานที่มีผลเฉพาะภายใน Tenant เดียว
ตาราง auth_user_groups และ auth_user_user_permissions ไม่มีคอลัมน์ tenant_id
จึงระบุขอบเขตสำนักงานไม่ได้ และรองรับได้เฉพาะสิทธิ์ระดับระบบเท่านั้น
...
ตารางทั้งแปดไม่มี Foreign Key ชี้ออกไปนอก subject area นี้เลย มีแต่ตารางภายนอก
ที่อ้างอิงเข้ามาที่ Primary Key ของ auth_user และ auth_group ซึ่งระบุไว้ในหัวข้อ
ของตารางนั้น โดยไม่ได้วาดตารางภายนอกไว้ในแผนภาพ
...
auth_user.id ถูกอ้างอิงต่อจากตารางนอก subject area นี้อีกสามจุด ซึ่งไม่ได้วาดไว้
ในแผนภาพด้านบน

- tenant_users.user_id ใช้ ON DELETE RESTRICT จึงลบบัญชีที่ยังเป็นสมาชิกของ
  สำนักงานใดอยู่ไม่ได้ ต้องถอนสมาชิกภาพก่อน
- checkout_sessions.buyer_user_id ใช้ ON DELETE SET NULL การลบบัญชีจึงเก็บ
  ประวัติการสั่งซื้อไว้แต่ตัดผู้ซื้อออก
- pending_trial_applications.resulting_user_id ใช้ ON DELETE SET NULL
  ด้วยเหตุผลเดียวกัน
...
| phone | VARCHAR(16) | หมายเลขโทรศัพท์ ความยาวตามมาตรฐาน E.164 |
...
auth_group.id ถูกอ้างอิงต่อจากตารางนอก subject area นี้หนึ่งจุด ซึ่งไม่ได้วาดไว้
ในแผนภาพด้านบน

- tenant_groups.group_id ใช้ ON DELETE CASCADE การลบแถวใน auth_group จึงลบแถว
  ใน tenant_groups ที่ผูกอยู่ไปด้วย และ group_id ยังเป็น UNIQUE จึงบังคับว่า
  หนึ่ง Django Group เป็นของสำนักงานเดียวเท่านั้น
...
ตัวอย่างผลเสียถ้าใช้ผิด สมมติทนายคนหนึ่งเป็น Admin ที่สำนักงาน ก. และเป็นสมาชิก
ธรรมดาที่สำนักงาน ข. หากมอบบทบาท Admin ผ่านตารางนี้ เขาจะกลายเป็น Admin ที่
สำนักงาน ข. ทันทีด้วย ซึ่งเป็นช่องโหว่ยกระดับสิทธิ์ข้ามสำนักงาน

ปัจจุบัน MatterSolv ยังไม่ได้นิยาม Group ระดับระบบไว้เลย สิทธิ์ของทีมงานผู้ให้บริการ
ใช้ auth_user.is_superuser และ auth_user.is_staff แทน ตารางนี้จึงเป็นตารางที่ Django
สร้างไว้ตามมาตรฐานและสงวนไว้เฉย ๆ ยังไม่มีการใช้งานจริงในระบบ
...
ตารางนี้สงวนไว้เช่นเดียวกับ auth_user_groups และยังไม่มีการใช้งานจริงในระบบ
...
- [Tenant & Authorization Tables](/docs/database/tenant-authorization) — ตารางฝั่งสำนักงาน

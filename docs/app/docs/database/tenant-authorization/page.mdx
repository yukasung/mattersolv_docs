...
- status มี CHECK จำกัดค่าไว้สามค่า และผูกกับ archived_at แบบสองทางเหมือน tenants
  คือ status เป็น archived ก็ต่อเมื่อ archived_at ไม่เป็น NULL
...
เงื่อนไขสองทางของ status กับ archived_at สำคัญเป็นพิเศษกับตารางนี้ เพราะการถอน
สมาชิกภาพคือการตัดสิทธิ์เข้าใช้งาน ถ้าตั้ง archived โดยลืมบันทึกเวลา หรือบันทึกเวลา
ไว้แต่สถานะยังเป็น active จะไม่มีทางรู้ได้ว่าสมาชิกคนนั้นถูกถอนสิทธิ์เมื่อใด
...
สำนักงานเดียวกัน และจำกัดความยาวไว้ไม่เกิน 50 ตัวอักษร

เพดาน 50 ตัวอักษรของ code ไม่ใช่ค่าที่ตั้งขึ้นลอย ๆ เพราะ provisioning ตั้ง
auth_group.name เป็นรูปแบบ tenant_uuid ตามด้วยทวิภาคและ code ลงในคอลัมน์
VARCHAR(150) UUID กิน 36 ตัวอักษรบวกทวิภาคอีกหนึ่ง จึงเหลือให้ code ได้มากสุด
113 ตัวอักษร ถ้า code ยาวเกินนั้นการสร้าง Group จะล้มเหลวตอน insert ค่า 50 จึง
เป็นเพดานที่ปลอดภัยและตรงกับมาตรฐาน code ของ departments กับ job_positions
...
postal_code มี CHECK บังคับว่าต้องเป็นตัวเลขห้าหลักพอดี เพราะ VARCHAR(5) เพียง
อย่างเดียวยังรับค่าอย่าง abcde ได้ employee_addresses.postal_code ใช้กฎเดียวกัน
สองตารางนี้ต้องแก้ไปด้วยกันเสมอ
